...
export async function getDossierOmgang(
    ...
    context.log('GET Dossier Omgang endpoint called');
    ...
    try {
        ...
        if (USE_REPOSITORY_PATTERN) {
            ...
            // Get all omgang records for this dossier with lookup data
            const omgangRecords = await omgangRepository.findByDossierId(dossierId);

            return createSuccessResponse(omgangRecords);
        } else {
            // Legacy path: use database service
            return createErrorResponse('Legacy omgang retrieval not implemented', 501);
        }
    } catch (error) {
        context.error('Error retrieving omgang records:', error);

        return createErrorResponse(
            error instanceof Error ? error.message : 'Failed to retrieve omgang records',
            500
        );
    } finally {
